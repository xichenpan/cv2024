...
class LCMDenoiseScheduler(SchedulerMixin, ConfigMixin):
    """
    ...
    def set_timesteps(
        ...
        if timesteps is not None:
            ...
            if timesteps[0] >= self.config.num_train_timesteps:
                raise ValueError(f"`timesteps` must start before `self.config.train_timesteps`:" f" {self.config.num_train_timesteps}.")
    ...
    def step(
        ...
        if self.config.prediction_type == "epsilon":  # noise-prediction
            ...
            raise ValueError(f"prediction_type given as {self.config.prediction_type} must be one of `epsilon`, `sample` or" " `v_prediction` for `LCMScheduler`.")
...
class FlowMatchEulerDiscreteScheduler(SchedulerMixin, ConfigMixin):
    ...
    def __init__(
        ...
        sigmas[1:] = sigmas[1:] * cut_off_sigma
        ...
        self.raw_timesteps = self.timesteps.clone()
        self.raw_sigmas = self.sigmas.clone()
    ...
    def set_timesteps(
        ...
        if sigmas is None:
            ...
            indices = [int(i * step_size) for i in range(num_inference_steps)]
            sigmas = self.raw_sigmas[indices]
            timesteps = self.raw_timesteps[indices]

        self.timesteps = timesteps.to(device=device)
        self.sigmas = torch.cat([sigmas.to(dtype=torch.float32, device=device), torch.zeros(1, device=sigmas.device)])
