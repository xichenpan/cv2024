...
class GaussianDiffusion(nn.Module):
    def __init__(self, alphas, alphas_cumprod, betas):
        ...
        self.register_buffer('sqrtalphas_cumprod', torch.sqrt(self.alphas_cumprod), persistent=False)
        self.register_buffer('sqrt_one_minusalphas_cumprod', torch.sqrt(1.0 - self.alphas_cumprod), persistent=False)
        self.register_buffer('log_one_minusalphas_cumprod', torch.log(1.0 - self.alphas_cumprod), persistent=False)
        self.register_buffer('sqrt_recipalphas_cumprod', torch.sqrt(1.0 / self.alphas_cumprod), persistent=False)
        self.register_buffer('sqrt_recipm1alphas_cumprod', torch.sqrt(1.0 / self.alphas_cumprod - 1), persistent=False)
        ...
        ) if len(self.posterior_variance) > 1 else torch.tensor([]), persistent=False)
